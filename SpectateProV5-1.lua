...
-- ══════════════════════════════════════════════════════════════════
-- 🔔 إشعار خروج اللاعب — يمين الشاشة مع صورة + اسم + يوزر
-- ══════════════════════════════════════════════════════════════════
local function showPlayerLeaveNotif(player)
    -- احذف أي إشعار قديم
    for _, child in ipairs(gui:GetChildren()) do
        if child.Name == "LeaveNotifFrame" then child:Destroy() end
    end

    local notif                  = Instance.new("Frame")
    notif.Name                   = "LeaveNotifFrame"
    notif.Size                   = UDim2.fromOffset(260, 74)
    notif.AnchorPoint            = Vector2.new(1, 0)
    notif.Position               = UDim2.new(1, 80, 0, 12)  -- تبدأ من خارج الشاشة
    notif.BackgroundColor3       = Color3.fromRGB(5, 12, 38)
    ...
    notifStroke.Color           = Color3.fromRGB(255, 120, 50)

    -- شريط لون أعلى برتقالي
    local topAccent              = Instance.new("Frame", notif)
    topAccent.Size               = UDim2.new(1, 0, 0, 3)
    topAccent.BackgroundColor3   = Color3.fromRGB(255, 140, 30)
    topAccent.BorderSizePixel    = 0
    topAccent.ZIndex             = 91
    Instance.new("UICorner", topAccent).CornerRadius = UDim.new(0, 12)
    Instance.new("UIGradient", topAccent).Color = ColorSequence.new{
        ColorSequenceKeypoint.new(0,   Color3.fromRGB(255, 80, 30)),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 210, 100)),
        ColorSequenceKeypoint.new(1,   Color3.fromRGB(255, 80, 30))
    }

    -- إطار صورة اللاعب (44x44)
    local avatarFrame              = Instance.new("Frame", notif)
    avatarFrame.Size               = UDim2.fromOffset(44, 44)
    avatarFrame.Position           = UDim2.fromOffset(10, 15)
    avatarFrame.BackgroundColor3   = Color3.fromRGB(10, 25, 70)
    avatarFrame.BorderSizePixel    = 0
    avatarFrame.ZIndex             = 92
    Instance.new("UICorner", avatarFrame).CornerRadius = UDim.new(0, 8)
    local avatarStroke = Instance.new("UIStroke", avatarFrame)
    avatarStroke.Thickness = 1.5
    avatarStroke.Color = Color3.fromRGB(255, 140, 30)
    ...
    -- أيقونة خروج فوق الصورة
    local exitIcon                  = Instance.new("TextLabel", notif)
    exitIcon.Size                   = UDim2.fromOffset(18, 18)
    exitIcon.Position               = UDim2.fromOffset(42, 8)
    exitIcon.BackgroundColor3       = Color3.fromRGB(200, 50, 20)
    exitIcon.BackgroundTransparency = 0
    exitIcon.BorderSizePixel        = 0
    exitIcon.Text                   = "✕"
    exitIcon.TextSize               = 10
    exitIcon.TextColor3             = Color3.fromRGB(255, 255, 255)
    exitIcon.Font                   = Enum.Font.GothamBold
    exitIcon.TextXAlignment         = Enum.TextXAlignment.Center
    exitIcon.ZIndex                 = 94
    Instance.new("UICorner", exitIcon).CornerRadius = UDim.new(1, 0)

    -- نص "غادر اللعبة"
    local leaveTitleLbl              = Instance.new("TextLabel", notif)
    leaveTitleLbl.Size               = UDim2.new(1, -62, 0, 16)
    leaveTitleLbl.Position           = UDim2.fromOffset(62, 12)
    leaveTitleLbl.BackgroundTransparency = 1
    leaveTitleLbl.TextSize           = 11
    leaveTitleLbl.Text               = "👋  غادر اللعبة"
    leaveTitleLbl.Font               = Enum.Font.GothamBold
    leaveTitleLbl.TextColor3         = Color3.fromRGB(255, 170, 60)
    leaveTitleLbl.TextXAlignment     = Enum.TextXAlignment.Left
    leaveTitleLbl.ZIndex             = 92

    -- اسم اللاعب (DisplayName)
    local displayNameLbl              = Instance.new("TextLabel", notif)
    displayNameLbl.Size               = UDim2.new(1, -62, 0, 18)
    displayNameLbl.Position           = UDim2.fromOffset(62, 28)
    ...
    -- يوزرنيم @username
    local usernameLbl                 = Instance.new("TextLabel", notif)
    usernameLbl.Size                  = UDim2.new(1, -62, 0, 14)
    usernameLbl.Position              = UDim2.fromOffset(62, 48)
    usernameLbl.BackgroundTransparency = 1
    usernameLbl.TextSize              = 10
    usernameLbl.Text                  = "@" .. player.Name
    usernameLbl.Font                  = Enum.Font.Gotham
    usernameLbl.TextColor3            = Color3.fromRGB(160, 190, 255)
    usernameLbl.TextXAlignment        = Enum.TextXAlignment.Left
    usernameLbl.ZIndex                = 92

    -- شريط تقدم زمني
    local pgBg              = Instance.new("Frame", notif)
    pgBg.Size               = UDim2.new(1, -14, 0, 2)
    pgBg.Position           = UDim2.new(0, 7, 1, -4)
    pgBg.BackgroundColor3   = Color3.fromRGB(20, 40, 90)
    pgBg.BorderSizePixel    = 0
    ...
    local pgBar              = Instance.new("Frame", pgBg)
    pgBar.Size               = UDim2.fromScale(1, 1)
    pgBar.BackgroundColor3   = Color3.fromRGB(255, 140, 30)
    pgBar.BorderSizePixel    = 0
    pgBar.ZIndex             = 93
    Instance.new("UICorner", pgBar).CornerRadius = UDim.new(1, 0)
    ...
    task.spawn(function()
        local dur = 4
        while notif.Parent and dur > 0 do
            TweenService:Create(notifStroke, TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                {Color=Color3.fromRGB(255, 200, 80), Transparency=0.1}):Play()
            task.wait(0.5)
            TweenService:Create(notifStroke, TweenInfo.new(0.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                {Color=Color3.fromRGB(255, 100, 30), Transparency=0}):Play()
            task.wait(0.5)
            dur = dur - 1
        end
    end)
    ...
    TweenService:Create(notif,
        ...
        {Position = UDim2.new(1, -10, 0, 12)}):Play()
    ...
    task.delay(4, function()
        ...
        TweenService:Create(notif,
            ...
            {Position = UDim2.new(1, 80, 0, 12), BackgroundTransparency = 1}):Play()
...
P.PlayerRemoving:Connect(function(removedPlayer)
    -- إشعار الخروج دائماً (سواء كنت تراقبه أو لا)
    showPlayerLeaveNotif(removedPlayer)
    ...
    if #newList == 0 then
        ...
        -- ← الجديد: إذا كنت تراقب هذا اللاعب تحديداً، انتقل تلقائياً للتالي
        if currentTarget == removedPlayer or currentTarget == nil then
            -- احسب idx الجديد ضمن القائمة الجديدة
            if idx > #newList then idx = 1 end
            spectating   = true
            local next   = newList[idx]
            if next then
                currentTarget = next
                startPlayerTimer()
                if next.Character then
                    local hum = next.Character:FindFirstChildOfClass("Humanoid")
                    if hum then
                        cam.CameraType    = Enum.CameraType.Custom
                        cam.CameraSubject = hum
                    end
                end
                showToast("🔄", "تم التبديل تلقائياً",
                    "تراقب الآن: " .. next.Name, C_BLUE1, 2.5)
            end
        else
            if idx > #newList then idx = 1 end
            spectating = true
