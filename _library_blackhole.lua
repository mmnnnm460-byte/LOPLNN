...
-- ══════════════════════════════════════════════════════════
--  BLACK HOLE BACKGROUND — دالة الثقب الأسود
-- ══════════════════════════════════════════════════════════
local function AddBlackHoleBackground(MainFrame)

	-- نتأكد إن الـ Frame جاهز وعنده حجم
	local FW = MainFrame.AbsoluteSize.X
	local FH = MainFrame.AbsoluteSize.Y
	if FW == 0 then FW = 560 end
	if FH == 0 then FH = 390 end
	...
	local CORE_R  = 28
	local DISK_MIN = 32
	local DISK_MAX = math.min(FW, FH) * 0.46

	-- دالة مساعدة محلية (مستقلة عن Create الأصلية — تتجنب تعارض الـ Parent)
	...
	local function inf(inst, goal, t)
		TweenService:Create(inst, TweenInfo.new(t or 1.5,
			Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), goal):Play()
	end

	-- ── الحاوية الرئيسية — ZIndex = 1 (خلف كل شيء) ──
	local BHHolder = N("Frame", {
		Name               = "BlackHoleHolder",
	...
	N("UICorner", { CornerRadius = UDim.new(0, 7), Parent = BHHolder })

	-- ── دالة لون الجسيم ──
	local function diskColor(t)
		if t < 0.08 then return Color3.fromRGB(255, 255, 255)
		elseif t < 0.20 then return Color3.fromRGB(180, 255, 255)
		elseif t < 0.40 then return Color3.fromRGB(0, 230, 255)
		elseif t < 0.62 then return Color3.fromRGB(0, 160, 255)
		elseif t < 0.80 then return Color3.fromRGB(0, 80, 200)
		else return Color3.fromRGB(0, 30, 110) end
	end

	-- ── حلقات التوهج الخارجية ──
	local glowDefs = {
		{ mult = 4.2, color = Color3.fromRGB(0, 20, 60),   trans = 0.94 },
		{ mult = 3.4, color = Color3.fromRGB(0, 40, 110),  trans = 0.91 },
		{ mult = 2.6, color = Color3.fromRGB(0, 70, 160),  trans = 0.87 },
		{ mult = 2.0, color = Color3.fromRGB(0, 120, 210), trans = 0.82 },
		{ mult = 1.55,color = Color3.fromRGB(0, 180, 255), trans = 0.74 },
		{ mult = 1.25,color = Color3.fromRGB(0, 220, 255), trans = 0.62 },
		{ mult = 1.08,color = Color3.fromRGB(180, 255, 255),trans = 0.42 },
	}

	local glowFrames = {}
	for _, gd in ipairs(glowDefs) do
		local sz = CORE_R * 2 * gd.mult
		local gf = N("ImageLabel", {
			Size               = UDim2.fromOffset(sz, sz * TILT_Y * 2.2),
			Position           = UDim2.new(0.5, -sz/2, 0.5, -(sz * TILT_Y * 2.2)/2),
			BackgroundTransparency = 1,
			Image              = "rbxassetid://6401143412",
			ImageColor3        = gd.color,
			ImageTransparency  = gd.trans,
			ZIndex             = 1,
			Parent             = BHHolder,
		})
		N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = gf })
		table.insert(glowFrames, { frame = gf, base = gd.trans })
	end

	-- ── قرص التراكم — جسيمات دائرية ──
	local NPARTS = 90
	local particles = {}

	for i = 1, NPARTS do
		local raw = math.random()
		local r   = DISK_MIN + math.pow(raw, 2.0) * (DISK_MAX - DISK_MIN)
		local ang = math.random() * math.pi * 2
		local spd = 0.00055 * math.pow(DISK_MIN / r, 1.5) * (0.88 + math.random() * 0.24)
		local t   = (r - DISK_MIN) / (DISK_MAX - DISK_MIN)
		local col = diskColor(t)
		local bri = 0.10 + (1 - t) * 0.90
		local sz  = math.max(1, math.floor(0.8 + (1 - t) * 2.2))
		local zOff = (math.random() - 0.5) * 0.035

		local dot = N("Frame", {
			Name               = "P" .. i,
			Size               = UDim2.fromOffset(sz, sz),
			BackgroundColor3   = col,
			BackgroundTransparency = 1 - bri * 0.65,
			BorderSizePixel    = 0,
			ZIndex             = 2,
			Parent             = BHHolder,
		})
		...
		table.insert(particles, {
			frame = dot, r = r, angle = ang, spd = spd,
			bri = bri, zOff = zOff, sz = sz, t = t,
		})
	end

	-- ── حلقة الفوتون ──
	local photonSzX = CORE_R * 2.28
	local photonSzY = photonSzX * TILT_Y * 2.0

	local photonGlow = N("Frame", {
		Size     = UDim2.fromOffset(photonSzX * 1.5, photonSzY * 1.5),
		Position = UDim2.new(0.5, -photonSzX * 0.75, 0.5, -photonSzY * 0.75),
		BackgroundTransparency = 1,
		ZIndex   = 3,
		Parent   = BHHolder,
	})
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = photonGlow })
	local pGlowStroke = N("UIStroke", {
	...
	local photonFrame = N("Frame", {
		Size     = UDim2.fromOffset(photonSzX, photonSzY),
		Position = UDim2.new(0.5, -photonSzX/2, 0.5, -photonSzY/2),
		BackgroundTransparency = 1,
		ZIndex   = 3,
		Parent   = BHHolder,
	})
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = photonFrame })
	local photonStroke = N("UIStroke", {
	...
	local innerFrame = N("Frame", {
		Size     = UDim2.fromOffset(photonSzX * 0.88, photonSzY * 0.88),
		Position = UDim2.new(0.5, -photonSzX*0.44, 0.5, -photonSzY*0.44),
		BackgroundTransparency = 1,
		ZIndex   = 3,
		Parent   = BHHolder,
	})
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = innerFrame })
	N("UIStroke", {
	...
	-- ── النواة السوداء ──
	local coreSzX = CORE_R * 2
	local coreSzY = coreSzX * TILT_Y * 2.1

	local coreShadow = N("ImageLabel", {
		Size               = UDim2.fromOffset(coreSzX * 2.2, coreSzY * 2.2),
		Position           = UDim2.new(0.5, -coreSzX * 1.1, 0.5, -coreSzY * 1.1),
		BackgroundTransparency = 1,
		Image              = "rbxassetid://6401143412",
	...
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = coreShadow })

	local coreFrame = N("Frame", {
		Size             = UDim2.fromOffset(coreSzX, coreSzY),
		Position         = UDim2.new(0.5, -coreSzX/2, 0.5, -coreSzY/2),
		BackgroundColor3 = Color3.fromRGB(0, 0, 0),
		BorderSizePixel  = 0,
		ZIndex           = 5,
		Parent           = BHHolder,
	})
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = coreFrame })
	local coreEdge = N("UIStroke", {
		Color           = Color3.fromRGB(0, 200, 255),
		Thickness       = 1.2,
		Transparency    = 0.70,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
	...
	-- ── القوس الأمامي (فوق النواة) ──
	local frontArcSzX = photonSzX * 1.02
	local frontArcSzY = photonSzY * 1.02

	local frontArc = N("Frame", {
		Size             = UDim2.fromOffset(frontArcSzX, frontArcSzY / 2),
		Position         = UDim2.new(0.5, -frontArcSzX/2, 0.5, 0),
		BackgroundTransparency = 1,
		ClipsDescendants = true,
		ZIndex           = 6,
		Parent           = BHHolder,
	})
	local frontArcInner = N("Frame", {
		Size             = UDim2.fromOffset(frontArcSzX, frontArcSzY),
		Position         = UDim2.fromOffset(0, 0),
		BackgroundTransparency = 1,
		ZIndex           = 6,
		Parent           = frontArc,
	})
	N("UICorner", { CornerRadius = UDim.new(1, 0), Parent = frontArcInner })
	local frontStroke = N("UIStroke", {
		Color           = Color3.fromRGB(0, 240, 255),
		Thickness       = 3,
		Transparency    = 0.05,
		ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
		Parent          = frontArcInner,
	})

	-- ── أنيميشن النبض ──
	inf(photonStroke, { Transparency = 0.25 }, 1.6)
	inf(pGlowStroke,  { Transparency = 0.72 }, 2.0)
	inf(frontStroke,  { Transparency = 0.20 }, 1.8)
	inf(coreEdge,     { Transparency = 0.85 }, 2.2)

	for i, gf in ipairs(glowFrames) do
		inf(gf.frame, { ImageTransparency = math.min(gf.base + 0.06, 0.98) }, 1.5 + i * 0.2)
	end

	-- ── حلقة الدوران ──
	local bhConn = RunService.Heartbeat:Connect(function(dt)
		if not BHHolder or not BHHolder.Parent then return end

		for _, p in ipairs(particles) do
			p.angle = p.angle + p.spd * dt * 60

			local cosA = math.cos(p.angle)
			local sinA = math.sin(p.angle)

			local px = cosA * p.r
			local py = sinA * p.r * TILT_Y + p.zOff * p.r

			local dist2 = px*px + (py / TILT_Y)*(py / TILT_Y)
			if dist2 < (CORE_R * 0.88)^2 then
				p.frame.Visible = false
			else
				p.frame.Visible = true
				local dop    = 0.40 + 0.60 * math.cos(p.angle + math.pi)
				local isBack = sinA > 0
				local alpha  = isBack
					and p.bri * dop * 0.25
					or  p.bri * (0.42 + 0.58 * dop)
				p.frame.ZIndex = isBack and 2 or 7
...
function lopklib:MakeWindow(Configs)
	...
	-- ══ تشغيل تأثير الثقب الأسود كخلفية ══
	-- نؤخره frame واحد حتى يكون AbsoluteSize جاهز
	task.defer(function()
		AddBlackHoleBackground(MainFrame)
	end)
