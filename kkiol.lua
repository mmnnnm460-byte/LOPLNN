...
local Theme = {
	...
	Stroke     = Color3.fromRGB(90, 90, 95),
...
function Library:CreateWindow(cfg)
	...
	-- أزرار صغيرة أعلى يمين البطاقة (إغلاق / ديسكورد / يوتيوب)
	...
	----------------------------------------------------------------
	-- إظهار/إخفاء الواجهة كلها (بطاقة + دوك): أيقونة عائمة قابلة للسحب
	----------------------------------------------------------------
	local hidden = false
	...
	corner(ToggleTab, 10)
	local toggleStroke = stroke(ToggleTab, Theme.Stroke, 0.5)
	addShadow(ToggleTab, 12, 0.6)

	local ToggleBtn = create("TextButton", {
		Text = "", AutoButtonColor = false, BackgroundTransparency = 1,
		Size = UDim2.new(1, 0, 1, 0), Parent = ToggleTab,
	})
	local toggleIcon = icon("eye", 16, false, Theme.Text)
	...
	function Window:CreateTab(tcfg)
		...
		-- صفحة محتوى التبويب (نفس نظام السابق: pageWrap + page قابل للتمرير)
		...
		local function select()
			...
			for _, t in Window.Tabs do
				...
				tween(t._dockBtn, TI, { BackgroundTransparency = 1 })
				tween(t._dockIcon, TI, { TextColor3 = Theme.SubText })
			end
			Window._current = Tab
			pageWrap.Visible = true
			pageWrap.GroupTransparency = 1
			pageWrap.Position = UDim2.new(0, 0, 0.04, 0)
		...
		dockBtn.MouseEnter:Connect(function()
			if Window._current ~= Tab then tween(dockBtn, TI, { BackgroundTransparency = 0.55 }) end
		end)
		dockBtn.MouseLeave:Connect(function()
			if Window._current ~= Tab then tween(dockBtn, TI, { BackgroundTransparency = 1 }) end
		end)
		dockBtn.Activated:Connect(select)

		Tab._dockBtn, Tab._dockIcon, Tab._page, Tab._wrap, Tab._select = dockBtn, dockIcon, page, pageWrap, select
		...
		function Tab:CreateButton(bcfg)
			...
			corner(btnEl, 8); stroke(btnEl, Theme.Stroke, STROKE_T)
			create("TextLabel", {
