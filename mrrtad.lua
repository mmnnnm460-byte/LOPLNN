-- ║         ORK HUB  |  White & Silver Edition    ║
-- ║         Theme: White / Silver / Dark          ║
-- ║         Version: 3.0.0                        ║
-- ║         GamepassId: 105840902987664            ║
...
function lopklib:SetTheme(NewTheme)
	...
	SaveJson("ork hub V3.json", lopklib.Save)
...
function lopklib:MakeWindow(Configs)
	...
	local WTitle    = Configs[1] or Configs.Name  or Configs.Title    or "HUB V3.0"
	local WMiniText = Configs[2] or Configs.SubTitle or "ORK HUB || Brookhaven"

	-- ORK Hub GamepassId (كود الكوكب)
	local ORK_GAMEPASS_ID = 105840902987664
	...
	-- Skull Logo on the left of TopBar
	local SkullLogo = Create("ImageLabel", TopBar, {
		Size               = UDim2.fromOffset(20, 20),
		Position           = UDim2.new(0, 6, 0.5),
		AnchorPoint        = Vector2.new(0, 0.5),
		BackgroundTransparency = 1,
		Image              = "rbxassetid://10734962068",
		ImageColor3        = Color3.fromRGB(255, 255, 255),
		ZIndex             = 5,
		Name               = "SkullLogo"
	})
	-- Animate skull color white/silver pulse
	task.spawn(function()
		local a = 0
		while TopBar and TopBar.Parent do
			a = (a + 3) % 360
			local w = (math.sin(math.rad(a)) + 1) / 2
			local brightness = math.floor(180 + w * 75)
			pcall(function()
				SkullLogo.ImageColor3 = Color3.fromRGB(brightness, brightness, brightness)
			end)
			task.wait(0.04)
		end
	end)
