...
local function BuildParticleSystem(ParticleContainer, MainFrameRef)
	...
	local SpawnRate   = 0.18 -- سرعة التوليد
	...
	local function SpawnMeteor()
		...
		local sizeW = math.random(55, 90)   -- حجم مناسب للـ UI
		local sizeH = math.floor(sizeW * 1.4)

		local meteor = Instance.new("ImageLabel")
		meteor.Name                  = "NAR_Meteor"
		meteor.Size                  = UDim2.fromOffset(sizeW, sizeH)
		meteor.Position              = UDim2.new(math.random(0, 100) / 100, 0, -0.4, 0)
		...
		connection = RunService.Heartbeat:Connect(function()
			if meteor and meteor.Parent then
				CreateTrail(meteor.Position, UDim2.fromOffset(sizeW / 12, sizeW / 12))
			else
