-- LOPK Library  |  Royal Black & Blue Edition
-- Version: 2.0.0
...
local ROYAL_BLACK  = Color3.fromRGB(5,  8,  18)
local ROYAL_BLACK2 = Color3.fromRGB(8,  12, 25)
local ROYAL_BLUE   = Color3.fromRGB(25, 70, 185)
local ROYAL_BLUE2  = Color3.fromRGB(60, 120, 220)
local ROYAL_STROKE = Color3.fromRGB(25, 70, 185)
local ROYAL_TEXT   = Color3.fromRGB(200, 215, 255)
local ROYAL_DTEXT  = Color3.fromRGB(120, 145, 210)
local ROYAL_ACCENT = Color3.fromRGB(80, 140, 255)

local lopklib = {
	Themes = {
		Darker = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, ROYAL_BLACK),
				ColorSequenceKeypoint.new(0.50, ROYAL_BLACK2),
				ColorSequenceKeypoint.new(1.00, ROYAL_BLACK)
			}),
			["Color Hub 2"]      = ROYAL_BLACK2,
			["Color Stroke"]     = ROYAL_STROKE,
			["Color Theme"]      = ROYAL_BLUE2,
			["Color Text"]       = ROYAL_TEXT,
			["Color Dark Text"]  = ROYAL_DTEXT,
			["Color Dark Purple"]= ROYAL_BLUE2,
			["Color Hub 9"]      = ROYAL_BLACK,
			["Color Dark Greem"] = ROYAL_DTEXT
		},
		Dark = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, ROYAL_BLACK),
...
local function ButtonFrame(Container, Title, Description, HolderSize)
	...
	FStroke.Color = Color3.fromRGB(25, 55, 130)

	local LeftGlow = Create("Frame", Frame, {
		Size             = UDim2.new(0, 2, 0.6, 0),
		Position         = UDim2.new(0, 0, 0.2, 0),
		BackgroundColor3 = ROYAL_BLUE,
		BorderSizePixel  = 0,
	...
	while true do
		...
		for _, val in pairs(lopklib.Instances) do
			...
			if base and prop and prop ~= "" then
				pcall(function()
					inst[prop] = base:Lerp(ROYAL_BLUE2, wave * 0.35)
				end)
			end
		end
	end
...
function lopklib:MakeWindow(Configs)
	...
	local WMiniText = Configs[2] or Configs.SubTitle or "Royal Edition"
	...
	task.spawn(function()
		...
		while MainFrame and MainFrame.Parent do
			angle = (angle + 4) % 360
			local wave = (math.sin(math.rad(angle)) + 1) / 2
			local r = math.floor(10  + wave * 15)
			local g = math.floor(30  + wave * 40)
			local b = math.floor(120 + wave * 65)
			local r2 = math.floor(60  * wave)
			local g2 = math.floor(120 * wave)
			local b2 = math.floor(160 + wave * 95)
			pcall(function()
	...
	function Window:MinimizeBtn()
		...
		if Minimized then
			...
			MinimizeButton.ImageColor3 = ROYAL_DTEXT
	...
	function Window:MakeTab(paste, Configs)
		...
		task.spawn(function()
			...
			while TabSelect and TabSelect.Parent do
				...
				local r = math.floor(10  + wave * 50)
				local g = math.floor(30  + wave * 70)
				local b = math.floor(120 + wave * 100)
				pcall(function()
		...
		function Tab:AddToggle(Configs)
			...
			THStroke.Color = Color3.fromRGB(25, 55, 130)
			...
			ShineGrad.Color = ColorSequence.new({
				...
				ColorSequenceKeypoint.new(0.35, ROYAL_BLUE),
				ColorSequenceKeypoint.new(0.65, ROYAL_BLUE),
			...
			local Knob = Create("Frame", ToggleHolder, {
				...
				AnchorPoint      = Vector2.new(0.5, 0.5),
				BackgroundColor3 = Color3.fromRGB(35, 55, 120),
				BorderSizePixel  = 0,
				ZIndex           = 3,
			})
			...
			local function SetToggle(Val)
				...
				if Default then
					...
					CreateTween({Knob, "BackgroundColor3", ROYAL_BLUE2, 0.25})
					CreateTween({ToggleHolder, "BackgroundColor3", Color3.fromRGB(8, 18, 45), 0.25})
					THStroke.Color = ROYAL_BLUE
					task.delay(0.05, SpawnPulse)
				else
					CreateTween({Knob, "Position", UDim2.new(0, 11, 0.5, 0), 0.28})
					CreateTween({Knob, "BackgroundColor3", Color3.fromRGB(35, 55, 120), 0.25})
					CreateTween({ToggleHolder, "BackgroundColor3", ROYAL_BLACK2, 0.25})
					THStroke.Color = Color3.fromRGB(25, 55, 130)
				end
				task.wait(0.3)
				WaitClickT = false
		...
		function Tab:AddSlider(Configs)
			...
			SBStroke.Color = Color3.fromRGB(25, 55, 130)
			...
			IndGrad.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0, Color3.fromRGB(15, 45, 130)),
				ColorSequenceKeypoint.new(1, ROYAL_BLUE2),
			})
		...
		function Tab:AddTextBox(Configs)
			...
			SFStroke.Color = Color3.fromRGB(25, 55, 130)
			...
			SFGrad.Color = ColorSequence.new({
				ColorSequenceKeypoint.new(0,   Color3.fromRGB(0,0,0)),
				ColorSequenceKeypoint.new(0.3, ROYAL_BLUE),
				ColorSequenceKeypoint.new(0.7, ROYAL_BLUE),
				ColorSequenceKeypoint.new(1,   Color3.fromRGB(0,0,0)),
			})
			...
			local TextBoxInput = Create("TextBox", SelectedFrame, {
				...
				Font              = Enum.Font.GothamBold,
				TextScaled        = true,
				TextColor3        = ROYAL_TEXT,
				ClearTextOnFocus  = TClearText,
				PlaceholderText   = TPlaceholderText,
				PlaceholderColor3 = ROYAL_DTEXT,
				Text              = ""
			})

			TextBoxInput.Focused:Connect(function()
				CreateTween({SFStroke, "Color", ROYAL_BLUE2, 0.2})
			end)
			TextBoxInput.FocusLost:Connect(function()
				CreateTween({SFStroke, "Color", Color3.fromRGB(25, 55, 130), 0.2})
			end)
			...
			TextBoxInput.FocusLost:Connect(function()
				CreateTween({PencilIcon, "ImageColor3", ROYAL_DTEXT, 0.2})
			end)
			TextBoxInput.Focused:Connect(function()
				CreateTween({PencilIcon, "ImageColor3", ROYAL_BLUE2, 0.2})
			end)
		...
		function Tab:AddDiscordInvite(Configs)
			...
			if BannerImage == "" then
				Create("UIGradient", BannerArea, {
					Rotation = -15,
					Color    = ColorSequence.new({
						ColorSequenceKeypoint.new(0, ROYAL_BLACK),
						ColorSequenceKeypoint.new(1, ROYAL_BLACK2)
					})
				})
			end
			...
			if MembersOnline > 0 or TotalMembers > 0 then
				...
				if MembersOnline > 0 then
					CreateStatusIndicator(ROYAL_BLUE2, MembersOnline .. " Online")
				end
				if TotalMembers > 0 then
					CreateStatusIndicator(ROYAL_DTEXT, TotalMembers .. " Members")
				end
			end
