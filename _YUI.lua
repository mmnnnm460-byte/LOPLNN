-- ╔══════════════════════════════════════════════╗
-- ║         LOPK Library  |  WindUI Edition      ║
-- ║         Theme: Dark Blue / Slate             ║
-- ║         Version: 2.0.0  |  Key: jartto0115   ║
-- ╚══════════════════════════════════════════════╝
...
local function CreateKeySystem()
	-- إزالة أي نافذة مفتاح قديمة
	local old = CoreGui:FindFirstChild("WindKeySystem")
	if old then old:Destroy() end

	local KeyGui = Instance.new("ScreenGui")
	KeyGui.Name             = "WindKeySystem"
	...
	Card.BackgroundColor3 = Color3.fromRGB(14, 17, 23)
	...
	CardStroke.Color           = Color3.fromRGB(55, 90, 200)

	-- خط علوي ملون (gradient)
	local TopBar = Instance.new("Frame", Card)
	TopBar.Size             = UDim2.new(1, 0, 0, 3)
	TopBar.BorderSizePixel  = 0
	TopBar.BackgroundColor3 = Color3.fromRGB(55, 90, 200)
	TopBar.ZIndex           = 3
	Instance.new("UICorner", TopBar).CornerRadius = UDim.new(0, 14)
	local TopGrad = Instance.new("UIGradient", TopBar)
	TopGrad.Color = ColorSequence.new({
		ColorSequenceKeypoint.new(0,   Color3.fromRGB(50,  80, 210)),
		ColorSequenceKeypoint.new(0.5, Color3.fromRGB(100, 140, 255)),
		ColorSequenceKeypoint.new(1,   Color3.fromRGB(50,  80, 210)),
	})
	...
	LockIcon.Image              = "rbxassetid://10734887180"
	LockIcon.ImageColor3        = Color3.fromRGB(90, 130, 255)
	LockIcon.ZIndex             = 3

	-- عنوان
	local TitleLabel = Instance.new("TextLabel", Card)
	TitleLabel.Size               = UDim2.new(1, 0, 0, 20)
	TitleLabel.Position           = UDim2.fromOffset(0, 62)
	...
	SubLabel.Size               = UDim2.new(1, -30, 0, 14)
	SubLabel.Position           = UDim2.fromOffset(15, 86)
	SubLabel.BackgroundTransparency = 1
	SubLabel.Text               = "Enter your key to continue"
	SubLabel.TextColor3         = Color3.fromRGB(130, 145, 180)
	...
	InputFrame.Size             = UDim2.new(1, -30, 0, 36)
	InputFrame.Position         = UDim2.fromOffset(15, 110)
	InputFrame.BackgroundColor3 = Color3.fromRGB(22, 27, 38)
	InputFrame.BorderSizePixel  = 0
	InputFrame.ZIndex           = 3
	Instance.new("UICorner", InputFrame).CornerRadius = UDim.new(0, 8)
	local InputStroke = Instance.new("UIStroke", InputFrame)
	InputStroke.Color     = Color3.fromRGB(45, 55, 80)
	...
	KeyInput.PlaceholderColor3  = Color3.fromRGB(90, 105, 140)
	KeyInput.Text               = ""
	KeyInput.TextColor3         = Color3.fromRGB(220, 228, 255)
	...
	-- رسالة الخطأ
	local ErrorLabel = Instance.new("TextLabel", Card)
	ErrorLabel.Size               = UDim2.new(1, -30, 0, 12)
	ErrorLabel.Position           = UDim2.fromOffset(15, 149)
	ErrorLabel.BackgroundTransparency = 1
	ErrorLabel.Text               = ""
	ErrorLabel.TextColor3         = Color3.fromRGB(255, 80, 90)
	...
	ConfirmBtn.Position         = UDim2.fromOffset(15, 164)
	ConfirmBtn.BackgroundColor3 = Color3.fromRGB(55, 90, 200)
	...
	ConfirmBtn.MouseEnter:Connect(function()
		TweenService:Create(ConfirmBtn, TweenInfo.new(0.2), {
			BackgroundColor3 = Color3.fromRGB(70, 110, 230)
		}):Play()
	end)
	ConfirmBtn.MouseLeave:Connect(function()
		TweenService:Create(ConfirmBtn, TweenInfo.new(0.2), {
			BackgroundColor3 = Color3.fromRGB(55, 90, 200)
		}):Play()
	end)
	ConfirmBtn.MouseButton1Down:Connect(function()
		TweenService:Create(ConfirmBtn, TweenInfo.new(0.1), {
	...
	KeyInput.Focused:Connect(function()
		TweenService:Create(InputStroke, TweenInfo.new(0.2), {
			Color = Color3.fromRGB(70, 110, 230)
		}):Play()
	end)
	...
	local function CheckKey()
		...
		if enteredKey == VALID_KEY then
			...
			KEY_VERIFIED = true
			ErrorLabel.Text = ""
			ConfirmBtn.Text = "✓ Verified!"
			TweenService:Create(ConfirmBtn, TweenInfo.new(0.3), {
				BackgroundColor3 = Color3.fromRGB(30, 160, 100)
			}):Play()
			TweenService:Create(CardStroke, TweenInfo.new(0.3), {
				Color = Color3.fromRGB(30, 200, 110)
			}):Play()
			task.wait(0.9)
			-- اختفاء
			...
			TweenService:Create(Card, TweenInfo.new(0.06), {
	...
	ConfirmBtn.Activated:Connect(CheckKey)
...
-- ══════════════════════════════════════════════════════════
--   تشغيل Key System أولاً قبل أي شيء
-- ══════════════════════════════════════════════════════════
local keyPassed = CreateKeySystem()
if not keyPassed then return end

-- ══════════════════════════════════════════════════════════
--   المكتبة الرئيسية (WindUI Style)
-- ══════════════════════════════════════════════════════════
	...
	Themes = {
		Darker = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(10, 12, 18)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(16, 20, 30)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 12, 18))
			}),
			["Color Hub 2"]      = Color3.fromRGB(18, 22, 32),
			["Color Stroke"]     = Color3.fromRGB(55, 90, 200),
			["Color Theme"]      = Color3.fromRGB(80, 120, 255),
			["Color Text"]       = Color3.fromRGB(225, 232, 255),
			["Color Dark Text"]  = Color3.fromRGB(140, 155, 195),
			["Color Dark Purple"]= Color3.fromRGB(100, 140, 255),
			["Color Hub 9"]      = Color3.fromRGB(0,   0,   0),
			["Color Dark Greem"] = Color3.fromRGB(100, 140, 255)
		},
		Dark = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(12, 14, 22)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(18, 22, 34)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(12, 14, 22))
			}),
			["Color Hub 2"]      = Color3.fromRGB(20, 24, 36),
			["Color Stroke"]     = Color3.fromRGB(60, 95, 210),
			["Color Theme"]      = Color3.fromRGB(85, 125, 255),
			["Color Text"]       = Color3.fromRGB(228, 234, 255),
			["Color Dark Text"]  = Color3.fromRGB(145, 160, 200),
			["Color Dark Purple"]= Color3.fromRGB(105, 145, 255),
			["Color Hub 9"]      = Color3.fromRGB(0,   0,   0),
			["Color Dark Greem"] = Color3.fromRGB(105, 145, 255)
		},
		Purple = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(14, 10, 26)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(20, 15, 38)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(14, 10, 26))
			}),
			["Color Hub 2"]      = Color3.fromRGB(22, 18, 40),
			["Color Stroke"]     = Color3.fromRGB(110, 70, 220),
			["Color Theme"]      = Color3.fromRGB(140, 100, 255),
			["Color Text"]       = Color3.fromRGB(230, 225, 255),
			["Color Dark Text"]  = Color3.fromRGB(155, 145, 200),
			["Color Dark Purple"]= Color3.fromRGB(145, 110, 255),
			["Color Hub 9"]      = Color3.fromRGB(0,   0,   0),
			["Color Dark Greem"] = Color3.fromRGB(145, 110, 255)
		},
		Neon = {
			["Color Hub 1"]      = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8,  12, 20)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(14, 18, 28)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8,  12, 20))
			}),
			["Color Hub 2"]      = Color3.fromRGB(15, 18, 28),
			["Color Stroke"]     = Color3.fromRGB(40, 180, 220),
			["Color Theme"]      = Color3.fromRGB(60, 210, 255),
			["Color Text"]       = Color3.fromRGB(220, 240, 255),
			["Color Dark Text"]  = Color3.fromRGB(135, 175, 210),
			["Color Dark Purple"]= Color3.fromRGB(80, 220, 255),
			["Color Hub 9"]      = Color3.fromRGB(0,   0,   0),
			["Color Dark Greem"] = Color3.fromRGB(80, 220, 255)
		}
	},
	Info       = { Version = "2.0.0" },
...
--  Element Factories (WindUI style – rounded corners tighter, blue accent)
...
AddEle("Button", function(parent, props, ...)
	...
	New.MouseEnter:Connect(function()
		IsMouseOver = true
		CreateTween({New, "BackgroundTransparency", 0.30, 0.18})
	end)
	New.MouseLeave:Connect(function()
		IsMouseOver = false
		CreateTween({New, "BackgroundTransparency", 0, 0.18})
	end)
	New.MouseButton1Down:Connect(function()
		PlayClickSound()
		CreateTween({New, "Size",                  OriginalSize - UDim2.fromOffset(4, 2), 0.1})
		CreateTween({New, "BackgroundTransparency", 0.50, 0.1})
	end)
	New.MouseButton1Up:Connect(function()
		CreateTween({New, "Size",                  OriginalSize, 0.15})
		CreateTween({New, "BackgroundTransparency", IsMouseOver and 0.30 or 0, 0.15})
	end)
...
-- WindUI-style shimmer (أزرق/أبيض بدل الأصفر)
task.spawn(function()
	task.wait(3)
	local angle = 0
	local shimmerColor = Color3.fromRGB(200, 215, 255)
	while true do
		task.wait(0.04)
		angle = (angle + 3) % 360
		...
		for _, val in pairs(lopklib.Instances) do
			...
			if base and prop and prop ~= "" then
				pcall(function()
					inst[prop] = base:Lerp(shimmerColor, wave * 0.35)
				end)
			end
		end
	end
...
function lopklib:Notify(Configs)
	...
	Make("Corner", NFrame, UDim.new(0, 10))
	...
	local AccentLine = Create("Frame", NFrame, {
		...
		BorderSizePixel  = 0
	}) Make("Corner", AccentLine, UDim.new(0.5, 0))
	...
	local DescLabel = Create("TextLabel", NFrame, {
		...
		BackgroundTransparency = 1,
		Text               = Description,
		TextColor3         = Color3.fromRGB(145, 160, 200),
		TextSize           = 9,
		Font               = Enum.Font.Gotham,
		TextXAlignment     = "Left",
		TextWrapped        = true
...
local function BuildParticleSystem(ParticleContainer, MainFrameRef)
	...
	local function Spawn()
		...
		if #ActiveParticles >= Config.MaxParticles then return end
		local cs   = ParticleContainer.AbsoluteSize
		if cs.X <= 0 or cs.Y <= 0 then return end

		local sx   = math.random(10, math.max(10, cs.X - 10))
		local sy   = cs.Y + 15
		...
		-- WindUI: ألوان زرقاء متنوعة
		local blueVariants = {
			Color3.fromRGB(80,  120, 255),
			Color3.fromRGB(60,  150, 255),
			Color3.fromRGB(100, 160, 255),
			Color3.fromRGB(50,  100, 230),
		}
		local col = blueVariants[math.random(1, #blueVariants)]

		local P = Instance.new("Frame")
		P.Size                   = UDim2.fromOffset(size, size)
		P.Position               = UDim2.fromOffset(sx, sy)
		P.BackgroundColor3       = col
		P.BackgroundTransparency = 1
		P.BorderSizePixel        = 0
		P.Parent                 = ParticleContainer
		Create("UICorner", P, { CornerRadius = UDim.new(0.5, 0) })

		local G = Instance.new("Frame")
		G.Size                   = UDim2.new(1, 5, 1, 5)
		...
		G.ZIndex                 = -1
		G.Parent                 = P
		Create("UICorner", G, { CornerRadius = UDim.new(0.5, 0) })

		TweenService:Create(P, TweenInfo.new(0.3), { BackgroundTransparency = 0.2 }):Play()
		TweenService:Create(G, TweenInfo.new(0.3), { BackgroundTransparency = 0.75 }):Play()

		table.insert(ActiveParticles, {
			Frame = P, Glow = G, StartTime = tick(), Speed = spd
		})
	end
	...
	local function Update()
		for i = #ActiveParticles, 1, -1 do
			...
			if not p.Frame or not p.Frame.Parent then
				table.remove(ActiveParticles, i); continue
			end
			local elapsed = tick() - p.StartTime
			if elapsed >= Config.Lifetime or p.Frame.Position.Y.Offset < -20 then
				TweenService:Create(p.Frame, TweenInfo.new(0.3), { BackgroundTransparency = 1 }):Play()
				TweenService:Create(p.Glow,  TweenInfo.new(0.3), { BackgroundTransparency = 1 }):Play()
				task.delay(0.3, function() if p.Frame then p.Frame:Destroy() end end)
	...
	_G.LOPKParticleConnection = RunService.Heartbeat:Connect(function()
		Update()
		if MainFrameRef and MainFrameRef.Visible then
			if tick() - LastSpawn >= Config.SpawnRate then
				LastSpawn = tick(); Spawn()
			end
		end
	end)
...
function lopklib:MakeWindow(Configs)
	...
	local WMiniText = Configs[2] or Configs.SubTitle or "WindUI Edition"
	...
	local function BlueNotify(title, text, duration)
		...
		Frame.Size = UDim2.fromOffset(290, 62)
		...
		AccentBar.BackgroundColor3 = Color3.fromRGB(80, 130, 255)
		...
		TitleL.Position = UDim2.fromOffset(14, 7)
		TitleL.BackgroundTransparency = 1
		TitleL.Text = title
		TitleL.TextColor3 = Color3.fromRGB(210, 225, 255)
		TitleL.TextSize = 12
		TitleL.Font = Enum.Font.GothamBold
		TitleL.TextXAlignment = Enum.TextXAlignment.Left

		local DescL = Instance.new("TextLabel", Frame)
		DescL.Size = UDim2.new(1, -20, 0, 18)
		DescL.Position = UDim2.fromOffset(14, 30)
		DescL.BackgroundTransparency = 1
		DescL.Text = text
		DescL.TextColor3 = Color3.fromRGB(140, 160, 210)
		DescL.TextSize = 10
	...
	Make("Gradient", MainFrame, { Rotation = 145 })
	MakeDrag(MainFrame)
	local MainCorner = Make("Corner", MainFrame)

	-- WindUI: خلفية أيقونة ناعمة
	...
	local BlueGrad = Create("UIGradient", BlueOverlay, {
		...
		Color = ColorSequence.new({
			ColorSequenceKeypoint.new(0.00, Color3.fromRGB(8,  10, 18)),
			ColorSequenceKeypoint.new(0.50, Color3.fromRGB(14, 18, 30)),
			ColorSequenceKeypoint.new(1.00, Color3.fromRGB(8,  10, 18)),
		}),
	...
	function Window:MinimizeBtn()
		...
		if Minimized then
			...
			MinimizeButton.ImageColor3 = Color3.fromRGB(80, 130, 255)
	...
	function Window:Dialog(Configs)
		...
		Make("Gradient", Frame, { Rotation = 145 })
		Make("Corner",   Frame)
	...
	function Window:MakeTab(paste, Configs)
		...
		local Selected = InsertTheme(Create("Frame", TabSelect, {
			Size              = FirstTab and UDim2.new(0, 2, 0, 3) or UDim2.new(0, 2, 0, 14),
			Position          = UDim2.new(0, 0, 0.5),
			AnchorPoint       = Vector2.new(0, 0.5),
			BackgroundColor3  = Theme["Color Theme"],
		...
		local function Tabs()
			...
			CreateTween({LabelTitle, "TextTransparency", 0,   0.30})
			CreateTween({LabelIcon,  "ImageTransparency",0,   0.30})
			CreateTween({Selected,   "Size",             UDim2.new(0, 2, 0, 14), 0.30})
			CreateTween({Selected,   "BackgroundTransparency", 0, 0.30})
		end
		...
		function Tab:Disable()
			...
			CreateTween({LabelTitle, "TextTransparency", 0.4, 0.30})
			CreateTween({LabelIcon,  "ImageTransparency",0.4, 0.30})
			CreateTween({Selected,   "Size",             UDim2.new(0, 2, 0, 3),  0.30})
			CreateTween({Selected,   "BackgroundTransparency", 1, 0.30})
		end
		...
		function Tab:AddToggle(Configs)
			...
			local function SetToggle(Val)
				...
				if Default then
					CreateTween({Toggle,       "Position",         UDim2.new(1, 0, 0.5), 0.20})
					CreateTween({Toggle,       "BackgroundTransparency", 0, 0.20})
					CreateTween({Toggle,       "AnchorPoint",      Vector2.new(1, 0.5), 0.20})
					CreateTween({ToggleHolder, "BackgroundColor3", Color3.fromRGB(20, 40, 90), 0.20})
				else
					CreateTween({Toggle,       "Position",         UDim2.new(0, 0, 0.5), 0.20})
					CreateTween({Toggle,       "BackgroundTransparency", 0.7, 0.20})
					CreateTween({Toggle,       "AnchorPoint",      Vector2.new(0, 0.5), 0.20})
					CreateTween({ToggleHolder, "BackgroundColor3", Theme["Color Stroke"], 0.20})
				end
				WaitClickT = false
		...
		function Tab:AddDropdown(Configs)
			...
			local DropFrame = Create("Frame", NoClickFrame, {
				...
				ClipsDescendants   = true,
				Active             = true
			}) Make("Corner", DropFrame) Make("Stroke", DropFrame) Make("Gradient", DropFrame, {Rotation = 145})
		...
		function Tab:AddDiscordInvite(Configs)
			...
			Make("Corner", CardFrame, UDim.new(0, 10))
			...
			local BannerArea = Create("ImageLabel", CardFrame, {
				...
				ScaleType         = Enum.ScaleType.Crop
			}) Make("Corner", BannerArea, UDim.new(0, 10))
			...
			if MembersOnline > 0 or TotalMembers > 0 then
				...
				if MembersOnline > 0 then
					CreateStatusIndicator(Color3.fromRGB(50, 210, 130), MembersOnline .. " Online")
				end
				if TotalMembers > 0 then
					CreateStatusIndicator(Color3.fromRGB(90, 120, 200), TotalMembers .. " Members")
				end
			end
