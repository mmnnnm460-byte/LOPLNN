...
local Libary = {
	...
	["Color Dark Greem"] = Color3.fromRGB(0, 255, 128)     
		...
		BlackSilver = {
			...
			["Color Theme"] = Color3.fromRGB(192, 192, 192),
			["Color Text"] = Color3.fromRGB(255, 255, 255),
			["Color Dark Text"] = Color3.fromRGB(180, 180, 180),
			["Color Dark Purple"] = Color3.fromRGB(210, 210, 210),
			["Color Hub 9"] = Color3.fromRGB(0, 0, 0),
			["Color Dark Greem"] = Color3.fromRGB(192, 192, 192)
		}
	},
	Info = {
		Version = "1.2.0"
...
local Settings = Libary.Settings
local Flags = Libary.Flags

local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(Libary.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then Libary.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then Libary.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then Libary.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, Libary.Connection do
...
local function VerifyTheme(Theme)
	for name,_ in pairs(Libary.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = Libary.Themes[Libary.Save.Theme]

local function AddEle(Name, Func)
	Libary.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = Libary.Elements[Ele](Instance, props, ...)
	return Element
end
...
function Libary:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end
	
	Libary.Save.Theme = NewTheme
	SaveJson("redz library V5.json", Libary.Save)
	Theme = Libary.Themes[NewTheme]
	
	Comnection:FireConnection("ThemeChanged", NewTheme)

	for _,Val in pairs(Libary.Instances) do
...
function Libary:SetScale(NewScale)
...
function Libary:Notify(Configs)
...
function Libary:MakeWindow(Configs)
	...
	local UISizeX, UISizeY = unpack(Libary.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, Libary.Save.TabSize, 1, -TopBar.Size.Y.Offset),
...
_G.ParticleSystemConnection = RunService.Heartbeat:Connect(function()
	...
	ConnectSave(ControlSize1, function()
		if not Minimized then
			Libary.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
			SaveJson("redz library V5.json", Libary.Save)
		end
	end)
	
	ConnectSave(ControlSize2, function()
		Libary.Save.TabSize = MainScroll.Size.X.Offset
		SaveJson("redz library V5.json", Libary.Save)
	end)
...
function Window:AddMinimizeButton(Configs)
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			Libary.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(Libary.Tabs) do
	...
	function Window:MakeTab(paste, Configs)
		...
		local function Tabs()
			...
			table.foreach(Libary.Tabs, function(_,Tab)
		...
		FirstTab = true
		local Tab = {}
		table.insert(Libary.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(Libary.Options, {type = "Section", Name = SectionName, func = Section})
...
--Font 
		...
		function Tab:AddToggle(Configs)
			...
			local function SetToggle(Val)
				...
				if Default then
					CreateTween({Toggle, "Position", UDim2.new(1, -15, 0.5), 0.25})
					CreateTween({Toggle, "AnchorPoint", Vector2.new(1, 0.5), 0.25})
					CreateTween({Toggle, "BackgroundColor3", Color3.fromRGB(255, 255, 255), 0.25})
					CreateTween({ToggleHolder, "BackgroundColor3", Color3.fromRGB(25, 25, 25), 0.25, Wait or false})
				else
